...
function load_facts_flash(filename, shuffle_items, callback) {
    jQuery.get(filename, function (data) {
        flashcards = Papa.parse(data, {
            header: true,
            skipEmptyLines: "greedy"
        }).data;
        if (shuffle_items) {
...
function load_facts_pilot(filename) {
    
    jQuery.get(filename, function (data) {
        pilot_items = Papa.parse(data, {
            header: true,
            skipEmptyLines: "greedy"
        }).data;
...
function get_next_fact(rep, current_time, get_from_server = false) {
    ...
    if (rep == 0) {
        ...
        question.study = false;
    }
    
    
    console.log(question.study)
    return question;
...
function get_new_pilot_item(it) {
    question = facts[it];
    return question
}
