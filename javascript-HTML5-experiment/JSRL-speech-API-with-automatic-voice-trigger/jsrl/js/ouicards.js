...
;(function(exports) {
  ...
  function next() {
    ...
    console.log(ouicards.bucketA)
    console.log(ouicards.bucketB)
    console.log(ouicards.bucketC)
    // Show an answer from Bucket A, so long as it's not empty
    if (ouicards.bucketA.length !== 0) {
  ...
  function next_test(count_tested_C){
    resetBuckets();
    newQuestion = buildQuestionHTML(ouicards.currentBucket[count_tested_C]);
    newQuestion.test = true;
    return newQuestion;
  }
  ...
  function buildQuestionHTML(rawQuestion) {
    var questionEl, answerEl, pronEl, dirEl
    ...
    pronEl = document.createElement('p');
    pronEl.innerHTML = rawQuestion.pron;

    dirEl = document.createElement('p');
    dirEl.innerHTML = rawQuestion.dir;

    textEl = document.createElement('p');
    textEl.innerHTML = rawQuestion.text;

    idEl = document.createElement('p');
    idEl.innerHTML = rawQuestion.id;

    return {pron: pronEl, dir: dirEl, question: questionEl, answer: answerEl, text: textEl, id:idEl};
  }
...
// jQuery magic
  ...
  $.fn.ouicards = function() {
    ...
    this.find('ul').hide().children().each(function() {
      result.push({
        questionflash: $(this).find('.question').text(),
        answerflash: $(this).find('.answer').text()
      });
    });
